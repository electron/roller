...
describe('getBranchesTrackedByMain', () => {
  ...
  it('includes a branch whose scheduled major equals the rolled major', async () => {
    ...
    await expect(getBranchesTrackedByMain(mockOctokit, 156)).resolves.toEqual(['45-x-y']);
  });

  it('returns every matching supported branch', async () => {
    nock(url.origin).get(url.pathname).reply(200, fixture);

    await expect(getBranchesTrackedByMain(mockOctokit, 150)).resolves.toEqual([
      '43-x-y',
      '44-x-y',
      '45-x-y',
    ]);
  ...
  it('returns no branches if the rolled major is newer than every scheduled version', async () => {
    ...
    await expect(getBranchesTrackedByMain(mockOctokit, 157)).resolves.toEqual([]);
  });
  ...
  it('ignores schedule entries for unsupported branches', async () => {
    ...
    await expect(getBranchesTrackedByMain(mockOctokit, 140)).resolves.toEqual([
      '42-x-y',
      '43-x-y',
      '44-x-y',
      '45-x-y',
  ...
  it('throws if the schedule fetch fails', async () => {
    ...
    await expect(getBranchesTrackedByMain(mockOctokit, 154)).rejects.toThrowError(
